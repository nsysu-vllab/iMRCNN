...
class RPN(nn.Module):
    ...
    def label_and_sample_anchors(
        ...
        try:
            ...
            gt_boxes = [x.pred_boxes for x in gt_instances]
    ...
    def losses(
        ...
        #test_mask = gt_labels == -1
        #loss = F.binary_cross_entropy_with_logits(
            #cat(pred_objectness_logits, dim=1)[test_mask],
            #torch.zeros(cat(pred_objectness_logits, dim=1)[test_mask].size()).to(device=cat(pred_objectness_logits, dim=1)[test_mask].device),
            #reduction="sum",
        #)
        #objectness_loss = objectness_loss + 0.001*loss
