...
for name in names:
    ...
    for mask_name in mask_list: 
        ...
        if len(color)>2:
            ...
            ####example end
            """
            semantic_mask3 = np.zeros(res_size) #拿來生產細胞質的mask
            ...
            #為了新加cy mask增加的程式碼 start
            semantic_mask3 = np.maximum(semantic_mask3, mask)
            semantic_mask3 = (semantic_mask3>0)*20
            cv2.imwrite(dest_root+'instance_y/'+mask_id+'.bmp', semantic_mask3)
            #為了新加cy mask增加的程式碼 end
