...
args.add_argument('--iterations',type=int,default=20000) #20000 設5000有到.4093
...
class KD_RCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        _, detector_losses = self.roi_heads(images, features, proposals, old_features, old_proposals, old_model, batched_inputs, gt_instances, old_instances)
...
class KD_ROIHeads(StandardROIHeads):#StandardROIHeads
    def forward( self, images: ImageList, features: Dict[str, torch.Tensor], origin_proposals: List[Instances], old_features: Dict[str, torch.Tensor] = None, old_proposals: List[Instances] = None, old_model = None, batched_inputs: List[Dict[str, torch.Tensor]] = None, targets: Optional[List[Instances]] = None, old_targets: Optional[List[Instances]] = None) -> Tuple[List[Instances], Dict[str, torch.Tensor]]:
        ...
        if self.training and old_proposals is not None:
            ...
            losses.update(self.KD_forward_mask(features, proposals, old_proposals, batched_inputs, images))
    ...
    def KD_forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances], old_instances: List[Instances] = None, batched_inputs = None, images = None):
        """
        ...
        #print(features)
        return self.mask_head(features, instances, old_instances, batched_inputs, images)  
