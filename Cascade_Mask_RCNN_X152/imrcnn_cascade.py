...
class KD_CRCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        #ocfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_nuclei_CnewSa/", "model_final.pth")  # path to the model weight just trained
        ...
        #ocfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_nuclei_CASCADE_cAgnos/", "model_final.pth")  # path to the model weight just trained
        #ocfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_multistep/model_and_log_CP_step2_72_nP/", "model_final.pth")  # path to the model weight just trained
        #ocfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_step1_allClass/", "model_final.pth")  # path to the model weight just trained
        ...
        #print(old_instances)
        #print("old_instances: ", old_instances)
        #INFERENCE
        ...
        ###########################################################################
        #                                                                         #
        #     將 pseudo label 與 ground truth 交集的地方改成只選取 ground truth !  #
        #                                                                         #
        ###########################################################################
        new_oldMask = None
        ...
        if self.proposal_generator is not None:
            ...
            proposal_losses = {}
        #print("old_proposals", old_proposals)
        #print("proposals", proposals)
        #print("om_proposals", om_proposals)
...
class KD_CROIHeads(CascadeROIHeads):#StandardROIHeads
    def forward( self, images: ImageList, features: Dict[str, torch.Tensor], origin_proposals: List[Instances], old_features: Dict[str, torch.Tensor] = None, old_proposals: List[Instances] = None, om_proposals = None, old_model = None, batched_inputs: List[Dict[str, torch.Tensor]] = None, targets: Optional[List[Instances]] = None, old_targets: Optional[List[Instances]] = None) -> Tuple[List[Instances], Dict[str, torch.Tensor]]:
        ...
        #print("out_test", self.outKD)
        #print("fea_test", self.feaKD)
        #print("PseudoLabel", self.PseudoLabel)
    ...
    def old_label_and_sample_proposals(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            #print("imrcnn proposals_per_image1: ", proposals_per_image)
            ...
            #print("imrcnn proposals_per_image2: ", proposals_per_image)
    ...
    def KD_forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances], old_features: Dict[str, torch.Tensor] = None, old_instances: List[Instances] = None, batched_inputs = None, images = None, om_proposals = None, old_model = None):
        ...
        if self.mask_pooler is not None:
            ...
            old_boxes = [x.proposal_boxes if self.training and old_instances is not None else x.pred_boxes for x in new_o_instances] #會出現 tensor size錯誤在這裏
            #o_boxes = [x.proposal_boxes if self.training and old_instances is not None else x.pred_boxes for x in om_proposals] #good
        ...
        #print(features)
        #print(old_instances)
        #return self.mask_head(features, instances, old_instances, batched_inputs, images, old_features, new_o_instances)  
    ...
    def KD_forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], targets: Optional[List[Instances]] = None, old_features: Dict[str, torch.Tensor] = None, old_proposals: List[Instances] = None, old_targets: Optional[List[Instances]] = None, o_proposals = None, old_model = None):  
        ...
        if self.training and old_features is not None:
            ...
            #print("old_proposals", old_proposals)
            #print("new_o_proposals", new_o_proposals)
            ...
            for k in range(self.num_cascade_stages):
                ...
                """
                #新增的 Pooler feature loss 結束 ============================================================
                #===========================================================================================
                
                #===========================================================================================
                #新增的 non bridging feature loss ==========================================================
                #print(old_targets)
                """for old_tar in old_targets:
                    height_img, width_img = old_tar.image_size
                    break
                    #print(height_img, width_img)
                    #print(old_tar.get('pred_boxes'))
                    
                for fea, o_fea in zip(features, o_features):
                    #print(fea.shape)
                    mask_batch_pre = []
                    height, width = fea.size(2), fea.size(3)
                    #print('old_targets====================================================')
                    #print(old_targets)
                    ...
                    for i in old_tar.get('pred_boxes'):
                        #print(i)
                        #print('old_tar====================================================')
                        #print(i)
                    ...
                    fea_loss = fea_loss + (torch.pow(fea - o_fea, 2) * mask_batch_pre).sum() / norms_pre
                    
                fea_loss = fea_loss.mean()
                """
                #print('fea_l')
                #print(fea_l.sum())
                #print('mask_batch_pre=============')
                #for mask in mask_batch_pre:
                #    print(mask.shape)
                #新增的 feature loss 結束 =======================================================================
                #===========================================================================================
            ...
            #新增的 feature loss =======================================================================
            """features_loss = {
                "features_loss": 0.01*fea_loss/3,  #pooler feature loss 的 weight
            }"""
            
            """features_loss = {
                "features_loss": 0.01*fea_loss/(3*2),  #pooler feature loss 的 weight
            }
            """
    ...
    def old_match_and_label_boxes(self, proposals, stage, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            #print("proposals_per_image: ",len(proposals_per_image))
...
#cfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_nuclei_CnewSa/", "model_final.pth")  # path to the model weight just trained
...
#cfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_nuclei_CASCADE_cAgnos/", "model_final.pth")  # path to the model weight just trained
#cfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_multistep/model_and_log_CP_step2_72_nP/", "model_final.pth")  # path to the model weight just trained
#cfg.MODEL.WEIGHTS = os.path.join("/home/frank/Desktop/instance segmentation/SegPC-2021-main/model_and_log/model_and_log_step1_allClass/", "model_final.pth")  # path to the model weight just trained
...
#cfg.MODEL.ROI_MASK_HEAD.CLS_AGNOSTIC_MASK = 1

cfg.TEST.EVAL_PERIOD = 1000 #250
#cfg.STEP = 0
