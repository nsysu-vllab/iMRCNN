...
parser.add_argument('-nu','--nucleus',help="the label assigned to nucleus in the instance mask",type=int,default=255) 
parser.add_argument('-cy','--cyto',help="the label assigned to cytoplasm in the instance mask",type=int,default=20)   ######修改這個 default 的數值就可以單獨測試nu或cy;   nu = 40; cy = 20
                                                                                                                      ######其他不需要修改
...
for img in tqdm.tqdm(imgs):
    ...
    for ins in insts:
        ...
        if len(ins_nu) == 0:
            ins_nu = ';'.join([','.join([str(0),str(0)])])
        if len(ins_cy) == 0:
            ins_cy = ';'.join([','.join([str(0),str(0)])])
